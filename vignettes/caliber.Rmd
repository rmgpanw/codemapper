...
1.  Outlines the steps performed by `read_caliber_raw()` and `reformat_caliber_for_ukb()` to achieve this.
2.  Highlights issues to be aware of
...
By default, these cases are dealt with by setting argument `overlapping_disease_categories_csv = default_overlapping_disease_categories_csv()` in `reformat_caliber_for_ukb()`. This uses a csv file which has been manually annotated ('Y' in column 'keep') to indicate which disease category a code should belong to:
